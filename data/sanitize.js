...
fetch("https://api.wynncraft.com/v3/item/database?fullResult").then(res => res.json()).then(fullResult => {
  ...
  Object.entries(fullResult).forEach(([key, value]) => {
    if(value.type == "ingredient") {
      if(!value.requirements.skills.includes(process.argv.slice(2)[0])) {
        return;
      }
      let sanitized = {}
      let identifications = []
      ...
      sanitized.position_modifiers = {
        ...
        notTouching: ifNotUndefined(value, "ingredientPositionModifiers", () => getOrDefault(value.ingredientPositionModifiers, "notTouching", 0), 0)
      }
